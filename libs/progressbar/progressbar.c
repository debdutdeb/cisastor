...
typedef int pb_write_ret_t;
typedef int pb_write_len_t;
#elif defined(__linux__)
typedef long pb_write_ret_t;
typedef unsigned long pb_write_len_t;
#else
#error "Only macOS and Linux are supported"
#endif

static pb_write_ret_t __progressbar_write(void *cookie, const char *buf,
                                          pb_write_len_t n) {
  ...
  fprintf(stderr, "\r[%s] [%s] %.2f%%", pb->options->label,
          progressbar_build(pb, n), pb->percent);
  fflush(stderr);
...
FILE *progressbar_open(FILE *fp, ProgressBarOptions *options) {
  ...
  pb->bar_width =
      (int)win.ws_col - (int)strlen(options->label) - extra_character_length;
  if (pb->bar_width < 1) {
    pb->bar_width = 1;
  }
...
#else
  cookie_io_functions_t io = {.write = __progressbar_write,
                              .close = __progressbar_close};
